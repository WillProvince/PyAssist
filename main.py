...
def speak(): # Function to generate TTS Responses
    global speech_array
...
def stop_speech(): # Stops Speech if needed
    try:
...
def calibrate_mic(): # Calibrate Mic on Startup
    try:
...
def recognize_speech_from_mic(): # STT Handler
    try:
        with sr.Microphone() as source:
...
class EventHandler(AssistantEventHandler):
  ...
  def handle_requires_action(self, data, run_id):
    ...
    for tool in data.required_action.submit_tool_outputs.tool_calls:
      ...
      print(Fore.MAGENTA + f"Parsed Arguments:{arguments}")
      ...
      elif tool.function.name == "run":
        try:
            command = arguments.get("command", "")
            if not command:
                tool_outputs.append({"tool_call_id": tool.id, "output": "No command provided"})
            else:
                output = os.popen(command).read()
                
                trimmed_output = output[:2000] + "..." if len(output) > 2000 else output
                response = f"Command executed: {command}\nOutput:\n{trimmed_output}"
                
                tool_outputs.append({"tool_call_id": tool.id, "output": f"Response: {response}"})
                print(f"TOOL OUTPUT: Executed command: {command}")
        except:
            tool_outputs.append({"tool_call_id": tool.id, "output": f"Failure to run: {e}"})
